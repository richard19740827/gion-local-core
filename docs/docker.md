...
```bash
git clone https://github.com/richard19740827/gion-local-core.git
cd gion-local-core
cp .env.example .env
...
## Repo 路徑、資料路徑與 WebUI source

`/Users/max/gion-local-core` 這類路徑應該是此 launcher/config repository 的本機 clone 位置。它只存放 compose、bootstrap 與治理腳本；canonical Docker flow 不會把這個 repo 當作 WebUI source code mount 進 container。
