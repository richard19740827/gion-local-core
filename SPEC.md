...
`docs/gion-local-core-v26.md` 保存多節點本地核心的前瞻設計，包括主節點/工作節點拓撲、租約管理、窗口守衛、資源降載、救援鏈、契約驗證與審計引擎。此文件不是目前 canonical install source；現行 main branch 仍以根目錄 `docker-compose.yml` 的 single-container flow 作為正式安裝入口。
