...
## Messaging gateway quickstart

If Hermes WebUI is running but Telegram/Discord/Slack still looks like a hidden channel switch, use the [messaging gateway quickstart](docs/troubleshooting.md#messaging-gateway-channel-not-opening). It shows how to find the active `HERMES_HOME`/`HERMES_CONFIG_PATH`, keep bot tokens out of screenshots and git, restart the gateway, and verify `/api/gateway/status`.
